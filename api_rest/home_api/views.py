    ...
    if request.method == 'POST': 
        ...
        if dados_0:
            ...
            if valor_target == 'DF':
                ...
                for id_verificar in Prev_extendida.objects.all():
                    if id_verificar.cidade==data0 and validar_0==0:
                        validar_0+=1
                        cidade_count.append(id_verificar.cidade)
                validar=0
                validar_0=0
                            ...
                            for verificar_datas in datas_previsao:
                                if validar_0==0:
                                    for a in range(0, len(data_comp)):
                                        if verificar_datas.cidade==data0 and verificar_datas.data_prev == data_comp[a-1] and verificar_datas.data_prev != '00/00/00':
                                                #efetuar a verificação de valores
                                                print(f'Info do dia:{verificar_datas.data_prev} salvo')
                                        if verificar_datas.data_prev != '00/00/00' and len(cidade_count)==0:
