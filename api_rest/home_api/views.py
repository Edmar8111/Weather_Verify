...
def requisita_clima_atual(cidade_name, estado_sigla):
    id_cidade = 0
    #efetua a limpeza para gerar o novo dado
    parametro = {'localeId[]':''}
    requisita_post = requests.put('http://apiadvisor.climatempo.com.br/api-manager/user-token/e4c68be43778972f692558eabf4f47df/locales', json=parametro)
    if requisita_post.status_code == 200:
        print('Dados Zerados')
    else:
        print('error api_clima')
    
    #efetua a requisição da cidade especificada pelo usuario
    request_city_id = requests.get(f'https://apiadvisor.climatempo.com.br/api/v1/locale/city?name={cidade_name}&state={estado_sigla}&token=e4c68be43778972f692558eabf4f47df')
    if request_city_id.status_code == 200:
        print('Dados enviados')
        dados = request_city_id.json()
        print(request_city_id.json())
    
    requisita_clima = requests.get(f'http://apiadvisor.climatempo.com.br/api/v1/weather/locale/{id_cidade}/current?token=e4c68be43778972f692558eabf4f47df')
    if requisita_clima.status_code==200:
        print(requisita_clima.json())
