...
dados_realtime = {'clima':'','dia_or_noite':'','umidade':'','sensa_termica':''}

def requisita_clima_atual(cidade_name):
    requisita_api = requests.get(f'http://api.weatherapi.com/v1/current.json?key=5c39e50cba4a4a4fa3530609242011&q={cidade_name}&aqi=no')
    if requisita_api.status_code==200:
        dados_api = requisita_api.json()
        dados_realtime['clima'] = dados_api['current']['temp_c']
        dados_realtime['dia_or_noite'] = dados_api['current']['is_day']
        dados_realtime['umidade'] = dados_api['current']['humidity']
        dados_realtime['sensa_termica'] = dados_api['current']['feelslike_c']
    print(dados_realtime)
    if float(str(dados_realtime['sensa_termica'])[3:])>=5:
        dados_realtime['sensa_termica'] = float(dados_realtime['sensa_termica'])+0.5
    if float(str(dados_realtime['clima'])[3:])>=5:
        dados_realtime['clima'] = float(dados_realtime['clima'])+0.5
    print(dados_realtime)
def home_0(request):
    valor_datas=[]
    msg_error = ''
            ...
            if valor_target0 == 'DF' or data0=='':
                ...
                for a in valores.objects.all():
                    if cidade_raspagem == a.nome_cidade and dados_0 == a.sigla_estado:
                        ...
                        requisita_clima_atual(data0)
                        return render(request,'base.html', {
                            'maxima':dados.maxima,'maxima_diaria':int(dados_realtime['clima']),'sensa_termica':int(dados_realtime['sensa_termica']),'umidade':dados_realtime['umidade'],
                            'minima':dados.minima, 'sigla':dados.sigla_estado, 'cidade':dados.nome_cidade.upper,
                ...
                if validar_exec==0:
                    ...
                    print(dados_realtime)
                    sleep(2)
                    return render(request, 'base.html', {'maxima_diaria':int(dados_realtime['clima']),'sensa_termica':int(dados_realtime['sensa_termica']),'umidade':dados_realtime['umidade'],
                            'maxima':maxima_list[0], 'minima':minima_list[0], 'sigla':str(dados_0).upper,'cidade':str(cidade_raspagem).upper,
                            'dia_0':Prev_extendida.objects.get(pk=int(valor_datas[-6])),
                            'dia_1':Prev_extendida.objects.get(pk=int(valor_datas[-5])),
                            'dia_2':Prev_extendida.objects.get(pk=int(valor_datas[-4])),
                            'dia_3':Prev_extendida.objects.get(pk=int(valor_datas[-3])),
